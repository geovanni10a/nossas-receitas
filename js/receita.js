(function () {
  ...
  function getSiblings(recipe) {
    var siblings = window.NRStorage.getRecipesByCategory(recipe.categoriaId);
  ...
  function renderRecipeDetail(recipeId) {
    ...
    if (!recipe) {
      window.location.href = "livro.html?mensagem=" + encodeURIComponent("Receita não encontrada.");
      return "";
    }

    var category = window.NRStorage.getCategoryById(recipe.categoriaId);
    var siblings = getSiblings(recipe);

    return [
      '<section class="detalhe-receita">',
      '  <div class="topo-detalhe">',
      '    <a class="botao-secundario" href="livro.html?categoria=' + recipe.categoriaId + '">Voltar à categoria</a>',
      '    <div class="acoes">',
      siblings.previous ? '<a class="botao-secundario" href="livro.html?receita=' + siblings.previous.id + '&categoria=' + recipe.categoriaId + '">← Receita anterior</a>' : "",
      siblings.next ? '<a class="botao-secundario" href="livro.html?receita=' + siblings.next.id + '&categoria=' + recipe.categoriaId + '">Próxima receita →</a>' : "",
      ...
      '      <div class="receita-imagem"><img src="' + (recipe.foto || PLACEHOLDER) + '" alt="Foto da receita ' + escapeHtml(recipe.titulo) + '"></div>',
      '      <div>',
      ...
      '        <span>Porções: ' + escapeHtml(recipe.porcoes || "-") + '</span>',
      '        <span>Dificuldade: ' + escapeHtml(recipe.dificuldade || "Fácil") + '</span>',
      '      </div>',
      recipe.tags.length ? '      <div class="receita-tags">' + recipe.tags.map(function (tag) { return '<span class="receita-tag">#' + escapeHtml(tag) + '</span>'; }).join("") + '</div>' : "",
